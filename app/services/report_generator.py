...
class ReportGenerator:
    ...
    def _load_audit_data(self, audit_run_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for response in responses:
                ...
                for brand, mention_data in brand_mentions.items():
                    if brand in all_brands:
                        mentions_count = mention_data.get('mentions', 0)
                        sentiment_score = mention_data.get('sentiment_score', 0.0)
                        
                        platform_stats[platform]['brand_mentions'][brand] += mentions_count
                        platform_stats[platform]['avg_sentiment'][brand].append(sentiment_score)
                        ...
                        brand_performance[brand]['total_mentions'] += mentions_count
                        brand_performance[brand]['platforms'].add(platform)
                        brand_performance[brand]['sentiment_scores'].append(sentiment_score)
            ...
            return {
                ...
                'date_range': {
                    ...
                    'end': audit_run.completed_at or audit_run.started_at
                }
            }
    ...
    def _create_title_page(self, data: Dict[str, Any]) -> List:
        ...
        metadata = [
            ...
            ["Audit Period:", f"{start_date.strftime('%B %d')} - {end_date.strftime('%B %d, %Y')}"],
    ...
    def _create_executive_summary(self, data: Dict[str, Any]) -> List:
        ...
        market_share = (client_mentions / total_mentions * 100) if total_mentions > 0 else 0
    ...
    def _create_platform_analysis(self, data: Dict[str, Any]) -> List:
        ...
        for platform, stats in data['platform_stats'].items():
            story.append(Paragraph(f"{platform.title()} Performance", self.heading_style))
    ...
    def _generate_insights(self, data: Dict[str, Any]) -> List[str]:
        ...
        if best_platform:
            insights.append(f"Strongest performance on {best_platform} with {max_mentions} mentions")
