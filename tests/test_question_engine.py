...
class TestQuestionEngine:
    ...
    @pytest.mark.parametrize("industry,competitors", [
    ...
    def test_generate_questions_with_categories(
        ...
        question_categories = {q["category"] for q in questions if q["category"] != "industry_specific"}
    ...
    def test_generate_questions_question_types(
        ...
        competitor_specific = [q for q in questions if q["type"] == "competitor_specific"]
    ...
    def test_generate_questions_industry_specific(
        ...
        industry_questions = [q for q in questions if q["category"] == "industry_specific"]
    ...
    def test_empty_competitors_list(self, engine: QuestionEngine) -> None:
        ...
        competitor_questions = [q for q in questions if q["type"] == "competitor_specific"]
        assert len(competitor_questions) == 0
        
        alternative_questions = [q for q in questions if q["type"] == "alternative_seeking"]
    ...
    def test_question_structure_validation(
        ...
        required_fields = ["question", "category", "type", "industry", "client_brand", "competitors"]
        
        for question in questions:
            # Verify all required fields are present
            for field in required_fields:
                assert field in question, f"Missing field {field} in question: {question}"
            
            # Verify question text is not empty
            assert len(question["question"].strip()) > 0
            
            # Verify category is valid
            valid_categories = [cat.value for cat in QuestionCategory] + ["industry_specific"]
