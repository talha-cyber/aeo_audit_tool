...
def test_resolve_personas_with_presets_and_custom_with_output(
    ...
    print("Preset persona details:")
    print(f"  - Uses voice: {preset.voice}")
    print(f"  - Emotional anchor: {preset.emotional_anchor}")
    print(f"  - Role: {preset.role}")

    print("Custom persona details:")
    print("  - Custom combination of role + driver + contexts")
...
def test_resolve_personas_ignores_unknown_voice_with_output(
    extractor: PersonaExtractor,
) -> None:
    ...
    print(f"{'='*50}")
    print("Requesting: ['missing_voice', 'value_shopper']")
    print("Expected: Should ignore 'missing_voice' and return only 'value_shopper'")
...
def test_resolve_personas_falls_back_to_default_context_with_output(
    extractor: PersonaExtractor,
) -> None:
    ...
    print(f"{'='*50}")
    print("Requesting role 'advocate' with unknown context 'unknown_stage'")
    print("Expected: Should fall back to role's default context")
    ...
    print("\nAnalysis:")
    print("- Requested context: ['unknown_stage']")
    print(f"- Actual context: {personas[0].contexts}")
    print("- Fallback worked: Used role's default context instead")
